...
for SQUARE_SIZE in range(5, 6):
    ...
    for unique_penalty_weight, domain_wall_penalty_weight, row_penalty_weight, col_penalty_weight in penalty_weights:

        # check if this experiment has already been run
        if os.path.exists(f"annealing analysis/domain_wall/solutions_{SQUARE_SIZE}.npy"):

            # load the existing data
            existing_data = np.load(f"annealing analysis/domain_wall/solutions_{SQUARE_SIZE}.npy", allow_pickle=True)
            
            if len(existing_data) > 0:
                if [unique_penalty_weight, domain_wall_penalty_weight, row_penalty_weight, col_penalty_weight] in [list(i["penalty_weights"].values()) for i in existing_data]:
                    print(f"[EXPERIMENT {iteration}] - skipping experiment for square size {SQUARE_SIZE} with penalty weights {unique_penalty_weight, domain_wall_penalty_weight, row_penalty_weight, col_penalty_weight}")
                    iteration += 1
                    continue
        ...
        filename = f"Annealing Analysis/domain_wall/solutions_{SQUARE_SIZE}.npy"
